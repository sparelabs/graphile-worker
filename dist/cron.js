 ...
 */
function getBackfillAndUnknownItems(parsedCronItems, knownCrontabs) {
    const backfillItemsAndDates = [];
    const unknownIdentifiers = [];
    for (const item of parsedCronItems) {
        const known = knownCrontabs.find((record) => record.identifier === item.identifier);
        if (known) {
 ...
 */
function unsafeRoundToMinute(ts, roundUp = false) {
...
function makeJobForItem(item, ts, backfilled = false) {
    return {
        task: item.task,
        payload: {
            ...item.payload,
 ...
 * automatically.
 */
async function scheduleCronJobs(pgPool, escapedWorkerSchema, jobsAndIdentifiers, ts, useNodeTime) {
 ...
 */
async function registerAndBackfillItems({ pgPool, events, cron }, escapedWorkerSchema, parsedCronItems, startTime, useNodeTime) {
    // First, scan the DB to get our starting point.
    const { rows } = await pgPool.query(`SELECT * FROM ${escapedWorkerSchema}.known_crontabs`);
    const { backfillItemsAndDates, unknownIdentifiers } = getBackfillAndUnknownItems(parsedCronItems, rows);
    ...
    const largestBackfill = parsedCronItems.reduce((largest, item) => Math.max(item.options.backfillPeriod, largest), 0);
    ...
    if (largestBackfill > 0) {
        ...
        while (unsafeTs < startTime) {
            ...
            if (itemsToBackfill.length) {
                ...
                await scheduleCronJobs(pgPool, escapedWorkerSchema, itemsToBackfill, ts, useNodeTime);
            }
...
const runCron = (options, parsedCronItems, requirements) => {
    ...
    function stop(e) {
        if (!stopCalled) {
            ...
            logger.error("Graphile Worker internal bug in src/cron.ts: calling `stop()` more than once shouldn't be possible. Please report this.");
        }
    }
    async function cronMain() {
        ...
        await registerAndBackfillItems({ pgPool, events, cron: this }, escapedWorkerSchema, parsedCronItems, new Date(+start), useNodeTime);
        ...
        async function loop() {
            try {
                ...
                const roundedCurrentTimestamp = currentTimestamp - (currentTimestamp % ONE_MINUTE);
                /*
                ...
                if (roundedCurrentTimestamp < expectedTimestamp) {
                    logger.debug(`Graphile Worker Cron fired ${((expectedTimestamp - currentTimestamp) /
                        1000).toFixed(3)}s too early (clock skew?); rescheduling`, {
                        expectedTimestamp,
                        currentTimestamp,
                    });
                    ...
                    logger.debug(`Graphile Worker Cron fired too late; catching up (${Math.floor((currentTimestamp - expectedTimestamp) / ONE_MINUTE)}m${Math.floor(((currentTimestamp - expectedTimestamp) % ONE_MINUTE) / 1000)}s behind)`);
                ...
                if (jobsAndIdentifiers.length) {
                    ...
                    await scheduleCronJobs(pgPool, escapedWorkerSchema, jobsAndIdentifiers, ts, useNodeTime);
...
async function getParsedCronItemsFromOptions(options, releasers) {
    ...
    if (crontab) {
        ...
        if (firstItem) {
            if (!firstItem[interfaces_1.$$isParsed]) {
                throw new Error("Invalid `parsedCronItems`; you must use a helper e.g. `parseCrontab()` or `parseCronItems()` to produce this value.");
            }
        }
        return parsedCronItems;
    }
}
exports.getParsedCronItemsFromOptions = getParsedCronItemsFromOptions;
/**
